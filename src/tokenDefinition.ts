...
import {ZERO_ADDRESS} from '../const/tokens'
...
export class PairTokens {
  ...
  constructor(token0: ethereum.CallResult<Address>, token1: ethereum.CallResult<Address>) {
    if ( token0.reverted || token1.reverted ) {
      this.isValid = false;
      this.token0 = Address.fromString(ZERO_ADDRESS);
      this.token1 = Address.fromString(ZERO_ADDRESS);
    } else {
      this.isValid = true;
      this.token0 = token0.value
      this.token1 = token1.value
    }
  }
}
